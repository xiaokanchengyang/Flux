...
fn test_s3_read_write() {
    ...
    let test_bucket = std::env::var("TEST_S3_BUCKET")
        .unwrap_or_else(|_| "flux-test-bucket".to_string());
    let test_path = format!("s3://{}/test-file-{}.bin", test_bucket, uuid::Uuid::new_v4());
...
fn test_s3_seek() {
    let test_bucket = std::env::var("TEST_S3_BUCKET")
        .unwrap_or_else(|_| "flux-test-bucket".to_string());
    let test_path = format!("s3://{}/test-seek-{}.bin", test_bucket, uuid::Uuid::new_v4());
...
fn test_gcs_operations() {
    ...
    let test_bucket = std::env::var("TEST_GCS_BUCKET")
        .unwrap_or_else(|_| "flux-test-bucket".to_string());
    let test_path = format!("gs://{}/test-file-{}.bin", test_bucket, uuid::Uuid::new_v4());
    ...
    let mut read_data = Vec::new();
    reader.read_to_end(&mut read_data).expect("Failed to read from GCS");
...
fn test_azure_operations() {
    ...
    let test_container = std::env::var("TEST_AZURE_CONTAINER")
        .unwrap_or_else(|_| "flux-test".to_string());
    let test_path = format!("az://{}/test-file-{}.bin", test_container, uuid::Uuid::new_v4());
    ...
    writer.write_all(test_data).expect("Failed to write to Azure");
    drop(writer);
    
    let mut reader = CloudReader::new(&test_path).expect("Failed to create Azure reader");
    let mut read_data = Vec::new();
    reader.read_to_end(&mut read_data).expect("Failed to read from Azure");
