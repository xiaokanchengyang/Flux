...
use flux_core::archive::{create_secure_extractor, extractor::ExtractEntryOptions};
use flux_core::security::{sanitize_path, validate_symlink};
use flux_core::Error;
...
fn test_zip_path_traversal_protection() {
    ...
    assert!(
        extracted_count <= 2,
        "At most normal.txt and Windows-style path should be extracted"
    );
    assert!(
        blocked_count >= 4,
        "At least 4 malicious entries should be blocked"
    );
...
fn test_compression_bomb_detection() {
    use flux_core::security::{check_compression_ratio, DEFAULT_MAX_COMPRESSION_RATIO};
