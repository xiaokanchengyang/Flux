...
use egui::{Context, Ui, Color32, vec2, Rect, Response, Sense, Id, Widget};
...
impl Widget for FluxButton {
    fn ui(self, ui: &mut Ui) -> Response {
        let theme = ui.ctx().data(|d| d.get_temp::<FluxTheme>(Id::NULL).unwrap_or_default());
        ...
        if ui.is_rect_visible(rect) {
            ...
            // Draw text
            ui.painter().text(
                cursor + vec2(content_width / 2.0 - if self.icon.is_some() { (icon_size + icon_spacing) / 2.0 } else { 0.0 }, 0.0),
                egui::Align2::CENTER_CENTER,
                &self.text,
                egui::FontId::proportional(14.0),
                text_color,
...
impl Widget for DropZone {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        if ui.is_rect_visible(rect) {
            ...
            // Background
            let bg_color = theme.colors.panel_bg.lerp_to_gamma(
                theme.colors.primary.gamma_multiply(0.1),
                hover_animation
            );
            ui.painter().rect_filled(rect, theme.rounding * 2.0, bg_color);
            
            // Dashed border
            let border_color = theme.colors.text_weak.gamma_multiply(0.5).lerp_to_gamma(
                theme.colors.primary,
                hover_animation
            );
            ...
            ui.painter().text(
                ...
                theme.colors.primary.gamma_multiply(0.7 + 0.3 * hover_animation),
            );
        ...
        if ui.is_rect_visible(rect) {
            ...
            if self.progress > 0.0 {
                let progress_rect = Rect::from_min_size(
                    rect.min,
                    vec2(rect.width() * self.progress, height),
                );
                
                // Gradient effect
                let gradient_id = ui.make_persistent_id("progress_gradient");
                let animation = ui.ctx().animate_value_with_time(
                    gradient_id,
                    self.progress,
                    0.3
                );
                ...
                if self.progress < 1.0 && self.progress > 0.0 {
                    ...
                    let shimmer_rect = Rect::from_min_max(
                        ...
                        egui::pos2((shimmer_x + 20.0).min(progress_rect.max.x), progress_rect.max.y),
                    );
