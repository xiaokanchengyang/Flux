...
pub fn draw_sync_view(
    ...
    ui.horizontal(|ui| {
        ...
        let target_text = target_archive
            .as_ref()
            .and_then(|p| p.to_str())
    ...
    ui.horizontal(|ui| {
        ...
        if ui
            .add_enabled(
                can_start,
                egui::Button::new(button_text).min_size(egui::vec2(180.0, 35.0)),
            )
            .clicked()
        {
        ...
        if existing_manifest.is_some()
            && ui
                .add_enabled(!is_busy, egui::Button::new("📋 View Manifest"))
                .clicked()
        {
