...
use flux_lib::archive::extractor::ArchiveEntry;
use crate::theme::FluxTheme;
use super::browser_view::{BrowserState, format_size, get_file_icon};
...
pub fn draw_table_view(
    ...
    flatten_tree(&state.tree, &mut flat_entries, &state.search_filter, state.show_hidden, 0);
        ...
        header.col(|ui| {
            ...
            let all_selected = !flat_entries.is_empty() && 
                flat_entries.iter().all(|(_, entry, _)| {
                    entry.as_ref().map_or(false, |e| !e.is_dir && state.selected.contains(&e.path))
                });
