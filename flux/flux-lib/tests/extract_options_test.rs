...
use flux_lib::{pack, extract_with_options, ExtractOptions};
use std::fs;
...
fn test_overwrite_option() {
    ...
    assert_eq!(content, "Original content", "File should have been overwritten");
}
...
fn test_strip_components() {
    ...
    assert!(!extract_dir.join("source").exists(), "source directory should not exist");
    assert!(extract_dir.join("root.txt").exists(), "root.txt should exist at top level");
    assert!(extract_dir.join("level1/mid.txt").exists(), "mid.txt should exist under level1");
    assert!(extract_dir.join("level1/level2/level3/deep.txt").exists(), "deep.txt should exist");
}
...
fn test_strip_components_deep() {
    ...
    assert!(extract_dir.join("level3/deep.txt").exists(), "deep.txt should exist under level3");
    assert!(extract_dir.join("mid.txt").exists(), "mid.txt should exist at top level");
