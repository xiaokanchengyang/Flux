...
use flux_lib::{pack_with_strategy, extract_with_options, PackOptions, ExtractOptions};
use std::fs;
...
fn test_unix_permissions_preserved() {
    ...
    assert_eq!(
        fs::metadata(&extracted_executable).unwrap().permissions().mode() & 0o777,
        0o755,
        "Executable permissions not preserved"
    );
    
    assert_eq!(
        fs::metadata(&extracted_readonly).unwrap().permissions().mode() & 0o777,
        0o444,
        "Read-only permissions not preserved"
    );
    
    assert_eq!(
        fs::metadata(&extracted_normal).unwrap().permissions().mode() & 0o777,
        0o644,
...
fn test_symlink_preserved() {
    ...
    assert!(extracted_symlink.symlink_metadata().unwrap().file_type().is_symlink());
...
fn test_directory_structure_preserved() {
    ...
    assert!(extract_dir.join("source/level1/level2/level3/deep.txt").exists());
