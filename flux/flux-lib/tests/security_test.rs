...
use flux_lib::security::{sanitize_path, validate_symlink, check_compression_ratio, check_extraction_size};
use std::path::Path;
...
mod secure_extraction_tests {
    use super::*;
    use flux_lib::archive::{create_secure_extractor, pack_with_strategy, PackOptions};
    use std::fs;
    ...
    fn test_secure_extraction_blocks_malicious_archive() {
        ...
        let entries: Vec<_> = extractor.entries(&archive_path)
            .unwrap()
            .collect();
